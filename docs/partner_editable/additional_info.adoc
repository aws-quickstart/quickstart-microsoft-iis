...
*These prerequisites are completed automatically as part of the Quick Start deployment.
...
If you want to modify this Quick Start see the https://github.com/aws-quickstart/quickstart-microsoft-iis/blob/main/templates/automation-parts.template.yaml[automation-parts.template.yaml] template. This AWS Cloudformation template uploads the MOF files and creates the automation documents that are triggered by the Auto Scaling group's lifecycle hooks. 
