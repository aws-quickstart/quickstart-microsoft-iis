...
[start=2]
. The AWS Region displayed in the upper-right corner of the navigation bar is where the {partner-product-short-name} network infrastructure will be deployed ({default_deployment_region} Region is the default.). Change it if necessary. 
...
. Review the parameters for the template, entering values where required and editing default values as needed. (See the link:#_parameter_reference[Parameter reference] section later in the guide.)
